...
def main():
    ...
    parser.add_argument("--bpm", type=float, default=None,
                        help="if set, append ', <bpm> BPM' to every prompt")
    ...
    with torch.inference_mode():
        ...
        src_mk = make_model_kwargs(
            build_cond_inputs(sam, pre_s + args.source_prompt + bpm_sfx, duration, 1, L, device),
            args.cfg_invert)
        ...
        for prompt in args.prompts:
            ...
            cond = build_cond_inputs(sam, pre_t + prompt + bpm_sfx, duration, 1, L, device)
