...
Honest read (MASTER §5 "judge by spread, not just corr"): the *direction* is rock-solid (0.986, monotonic), but *authority* at gain 48 is **moderate** — a 0.74→0.83 onset-strength swing across the full range. Gain 96 (top of the SA3-medium range) or a wider/later guidance window should widen it — knob-tuning, not redesign. The trained head + verifier are persisted alongside the render set.

## 8. The boundary (2026-06-24): beat/downbeat *don't* steer — trainability ≠ steerability

We trained **beat** and **downbeat** heads on the same recipe (loss 0.17 / 0.18, matching onset) and tried to steer them. Both ways failed:

- **Constant target** (gain 48): madmom beat-activation of the output stayed flat (0.0227→0.0223 across requested 0.0→0.2 — and slightly the *wrong* way); downbeat flat. No steering.
- **Time-varying pulse-train target** at 90/120/150 BPM (gain 96, no-BPM prompt): the guided outputs were **~identical to the unguided baseline** (wav |Δ| = 0.5–1% of amplitude) and **all read 161.5 BPM** (two estimators agree). The guidance **barely moved the latent at all**.
- **Confound checked and ruled out.** beat/downbeat are **soft** signals (madmom RNN probabilities), but the 100 Hz→10.767 Hz resampling **compresses** them: values top out at **~0.21** (beat p99 0.20 / max 0.214), not ~1.0. The first pulse target used peak **0.8** (+11σ, ~4× the data max) — an out-of-distribution ask that could alone explain a dead gradient. So we **re-ran with an in-distribution pulse (peak 0.20 = the data max)** — and it was **still a no-op** (wav |Δ| ~1%, tempo unchanged). So the null is **not** a target-amplitude artifact; the boundary is real.

**Why — and it's the real lesson.** The onset head (same code path, same gains) clearly moves the audio; the beat head is a no-op. The difference is the feature's *shape*: **onset_envelope is dense + continuous**, so every frame contributes a meaningful gradient and the latent moves; **beat/downbeat_activation is a sparse detector signal** (μ0.044, mostly ~0) — the head *reads* it (it trains fine), but its per-frame gradient toward a pulse is too weak and too local to **reorganize the global rhythmic structure**, so the latent stays put.

> **Training-free latent guidance steers *amount/density* features (onset density, RMS, brightness — strong dense gradient), and is a no-op on *sparse structural/timing* features (beat/downbeat placement).** Trainability ≠ steerability; the gap is whether the head's gradient can actually *reorganise* the latent.

This bounds the method honestly and connects back: §3's per-frame map predicts *readability*; §6 showed a temporal head reads rhythm; §7 showed onset *steers* — but §8 shows that steering only transfers to **dense** features. **Onset density is the steerable rhythm axis.** Beat/downbeat *placement* needs **trained conditioning that consumes a beat grid** (the Music-ControlNet / MuseControlLite path in the Sourcebook, Ch8/Ch11), not inference-time head guidance. (Higher gain than 96 untested, but the gradient is ~50–100× weaker than onset's → it would likely break coherence before imposing a clean grid; the issue is feature-type, not authority.) Null-result artifacts persisted alongside the render set.
