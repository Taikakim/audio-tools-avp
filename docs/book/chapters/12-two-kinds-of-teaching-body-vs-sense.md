...
You are going to bolt a small device onto the *outside* of the instrument. Not into its body; onto it. This device is a control head: a separate little network, trained apart from the generative model, whose entire job is to *listen* to the latent material as it forms and report one number back to you. How much energy is sitting in the low frequencies right now. How bright this moment is. How dense the events are across this stretch of time. It is a learned ear, tuned to hear one thing with precision — and it is not part of the instrument. The generative model does not know it is there.

By itself, that device is just a meter — a needle that twitches as the material forms. The second move is what makes it a *teaching*. During generation — during the actual carving walk from noise to music of Chapter 3 — the head reads the latent, compares what it reports against a target you set when you sat down to play, and *nudges the latent material* to narrow the gap between the two. Then the model reads the nudged material and carries on. Read, compare, nudge. Read, compare, nudge. The head sits *outside* the walk, not inside it: the model takes its step, and then, between steps, the head measures and corrects what the step produced. A loop closed around the outside of an instrument whose insides were never opened.

How tightly that loop closes is a tunable choice. You can let the head read and nudge at *every* sigma step — the finest grain of feedback, the latent corrected dozens of times across one walk — or batch it, intervening only on certain steps or within a chosen band of molten-ness. Tighter feedback holds the target more firmly; looser feedback leaves the model more room to be itself between corrections. There is no single right setting, and the difference is audible.

Here the closest analogy is the console: the head is a *real-time EQ driven by its own spectrum analyser*. The analyser watches the low band as the mix forms, sees it sagging beneath the line you drew, and the EQ pushes it back up — continuously, while the sound is still being made, not after. That is a control head, exactly: a learned analyser bonded to a corrective nudge. This is closed-loop control, and the difference from the first teaching is total. You are no longer setting a tendency and trusting it; you are measuring the actual material, in this actual run, and correcting it toward the target *as it forms*.

The full machinery of how a head is trained and what the two flavours of nudge actually do is the work of Chapter 13. But one piece of it belongs here, because it is the most overlooked tuning act in the whole practice: the **gain** — how hard the nudge pushes. A head reports a gap; the gain decides how forcefully you act on it. And the latent material does not have a fixed loudness across models. The large autoencoder's latent runs roughly *ten times hotter* in magnitude than the small one's — its numbers simply live at a bigger scale. A nudge sized correctly for one is, on the other, either a whisper the carving walk shrugs off, leaving you convinced the head does nothing, or a shove so violent it blows the material apart into noise. Same head, same target, same wiring — and the only thing you changed was a scaling factor matched to the latent's natural amplitude. Get it wrong in either direction and the loop appears broken when it is merely mistuned. This is the number that most often stands between a head that steers and a head that seems inert, and it is invisible unless someone tells you to look for it.

Does the nudge feel the same all the way down the walk? Not quite. Early on, when the material is molten (high sigma, Chapter 4), a nudge moves soft, unset stuff — it bends the whole trajectory of where the run is heading. Late, when the material has nearly set, the same nudge meets something stiffer and more committed; it adjusts a near-finished surface rather than redirecting the journey. The loop runs across the entire sigma walk, but *what* a correction accomplishes shifts as the material hardens under it — molten clay early, cured lacquer late.
...
- The closed loop corrects toward a target. But who sets that target, at what resolution, and must it be a single fixed number — or could the thing you correct toward be a shape that changes through time, a contour rather than a point?

- What happens if you do both at once: a LoRA that has leaned the body's natural tendencies one way, and a head correcting live toward a target the other way? Do they cooperate, fight, or open some third character of control that is neither purely set-and-forget nor purely measure-and-correct?
...
- The head reads the latent and nudges it; but the model is reading that same latent on its very next step. When your correction and the model's own momentum meet on the same material, do they add, cancel, or interact in ways you would need to watch for — and is that interaction itself a place a future control could live?

- If a head's nudge does something different at high sigma than at low — bending the whole trajectory early, polishing a near-set surface late — could you place a control deliberately in just one band of molten-ness, and what new kind of steering would that buy you?
